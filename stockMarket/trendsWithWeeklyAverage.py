...
def createModel():
    ...
    # layer architecture
    model.add(GRU(units = 512,input_shape = (None, 6), return_sequences=True,recurrent_dropout = 0.25))
...
def preprocessData(currentStockID, daysBack):
    ...
    # open sentimnet data
    sentimentDataFileName = 'GOOGLsentiment.csv'
    sentimentDataObj = pd.read_csv(sentimentDataFileName)
    sentimentDataValues = sentimentDataObj.values
    sentimentMap = parseSentimentWeekly(sentimentDataValues)
    
    searchTrendDataFileName = 'GOOGLsearchTrends.csv'
    trendData = pd.read_csv(searchTrendDataFileName)
    ...
    # insert sentiment values to correct dates
    x=0
    for i in range(len(training_set_scaled)):
        curDate = training_set_scaled[i, len(training_set_scaled[i,:]) - 1]
        curWeek = dateToWeek(curDate)
        
        if curWeek in sentimentMap:
            sentiments = sentimentMap[curWeek]
            
            for j in range(3):
                curSentiment = sentiments[j]
                training_set_scaled[i, j + 2] = curSentiment
        else:
            x +=1
            print x
            training_set_scaled[i, 2] = 0
            training_set_scaled[i, 3] = .5
            training_set_scaled[i, 4] = 0
...
def createWeeklyTrendMap(trendData):
    ...
    trendDates = trendData[:, 0]
    trends = trendData[:, 1]
    trends = np.reshape(trends, (len(trends), 1))
    trendDates = np.reshape(trendDates, (len(trendDates), 1))
    
    trendScaleData = trends
    trendScaler = MinMaxScaler(feature_range = (0, 1))
    trendScaler.fit(trendScaleData)
    trends = trendScaler.transform(trends)
    
    trends = np.append(trends, trendDates, axis=1)
    ...
    for i in range(len(trends)):
        ...
        curTrendVal = curRow[0]
        weekMap[curWeek][0] += curTrendVal

    
    # take the average of each value
    for week in weekMap:
        denominator = weekMap[week][1]

        weekMap[week][0] = (weekMap[week][0] / denominator)
